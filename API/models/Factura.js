  ...
  return new Promise(async (resolve, reject) => {
    try {
      let resultado =
        ...
        s.str_fecha, m.moneda_simbolo, sd.str_monto from factura f left join str_detalle sd natural join str s natural join moneda m on f.factura_nro = sd.factura_nro and f.factura_timbrado = sd.factura_timbrado inner join contrato con on f.contrato_nro = con.contrato_nro where con.licitacion_id = ${licitacionID} and con.contrato_nro = ${contratoNro} order by factura_fecha desc, factura_nro desc`)
