...
exports.apiGetContratoByEstado = async function (req, res) {
  try {
    let contratos = await Contrato.contratosByEstado(req.params.id, req.params.estado)
    let contratosOrdenado = ordenarResultado(contratos)
    res.json(contratosOrdenado)
  } catch (error) {
    res.status(500).send('Error')    
...
exports.apiGetContratosBySearch = async function (req, res) {
  try {
    let contratos = await Contrato.contratosBySearch(req.params.id, req.params.input)
    let contratosOrdenado = ordenarResultado(contratos)
    res.json(contratosOrdenado)
  } catch (error) {
    res.status(500).send('Error')    
