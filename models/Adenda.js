...
Adenda.contratoByNro = async function (licitacionID, contratoNro) {
  return new Promise(async (resolve, reject) => {
    try {
      let resultado =
...
Adenda.contratosByEstado = async function (licitacionID, estado) {
  let contratoObj = []
  let x = 0
...
Adenda.contratosBySearch = async function (licitacionID, input) {
  let contratoObj = []
  let x = 0
  return new Promise(async (resolve, reject) => {
    try {
...
Adenda.contratoResumen = async function (licitacionID, contratoNro) {
  return new Promise(async (resolve, reject) => {
    try {
      // adjudicacion normal
      let resultado = await pool.query(
...
Adenda.verAdenda = async function (licitacionID, contratoNro) {
  return new Promise(async (resolve, reject) => {
    try {
...
Adenda.prototype.addContrato = async function () {
  const {
    licitacion_id,
    activo,
    cumplimiento,
  ...
  return new Promise(async (resolve, reject) => {
    if (!this.errors.length) {
      try {
        let resultado = await pool.query(
          `INSERT INTO PUBLIC.ADENDA(ADENDA_NRO,
            CONTRATO_NRO,
            CONTRATO_YEAR,
            TIPO_CONTRATO_ID,
            ADENDA_FIRMA,
        ...
        if (monto) {
          ...
          await pool.query(
            `INSERT INTO ADENDA_DISMINUCION(RUBRO_ID,
              ADENDA_NRO,
              CONTRATO_NRO,
              CONTRATO_YEAR,
              TIPO_CONTRATO_ID,
              ADENDA_DISMINUCION_MONTO)
              VALUES (${nro},'${codigo}', ${nro}, ${contrato_nro}, ${contrato_year}, ${tipo_contrato_id}, ${monto})`
          )
        }
